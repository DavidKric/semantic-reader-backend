...
def test_generate_report(sample_document_result, temp_output_dir):
    ...
    assert len(stats) == 5  # Documents, Pages, Text Blocks, Tables, Figures
...
def test_empty_elements(temp_output_dir):
    ...
    assert stats[0].text == "1"  # 1 document
    assert stats[1].text == "1"  # 1 page
    assert stats[2].text == "0"  # 0 text blocks
    assert stats[3].text == "0"  # 0 tables
    assert stats[4].text == "0"  # 0 figures


def test_javascript_inclusion(sample_document_result, temp_output_dir):
    """Test that JavaScript is included in the report."""
    generator = HTMLReportGenerator(output_dir=temp_output_dir)
    report_with_js = generator.generate_report([sample_document_result])
