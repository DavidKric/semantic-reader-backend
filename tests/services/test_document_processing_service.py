...
async def test_update_document(document_service, sample_document):
    ...
    assert updated_document.updated_at > sample_document.updated_at
...
async def test_delete_document(document_service, sample_document):
    ...
    section = Section(document_id=sample_document.id, title="Test Section", section_type="content")
    para = Paragraph(document_id=sample_document.id, text="Test paragraph", page_number=1)
...
@patch('app.services.document_processing_service.DOCLING_AVAILABLE', True)
@patch.object(DocumentProcessingService, 'api_service')
async def test_parse_document_with_external_api(mock_api_service, document_service):
    ...
    mock_api_service.process_document.return_value = {
        ...
        "paragraphs": [{"text": "Para 1", "page_number": 1}],
        "sections": [{"title": "Section 1", "type": "heading"}]
    }
